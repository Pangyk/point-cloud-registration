...
def detect_folding(
    ...
    from robot.utils.img_visual_utils import compute_jacobi_map
    from robot.utils.utils import point_to_grid
...
def get_omt_mapping(
    ...
    from torch import Tensor as LazyTensor
    ...
    a_i, x_i = LazyTensor(source.weights.view(N, 1, 1).cpu()), LazyTensor(attr1.view(N, 1, -1).cpu())
    b_j, y_j = LazyTensor(target.weights.view(1, M, 1).cpu()), LazyTensor(attr2.view(1, M, -1).cpu())
    F_i, G_j = LazyTensor(F_i.view(N, 1, 1).cpu()), LazyTensor(G_j.view(1, M, 1).cpu())
    C_ij = (1 / p) * ((x_i - y_j) ** p).sum(-1)  # (N,M,1) cost matrix
    eps = blur ** p  # temperature epsilon
    # print(F_i.shape, G_j.shape, C_ij.shape, a_i.shape)
    F_i = F_i.expand(-1, M, -1)
    G_j = G_j.expand(N, -1, -1)
    C_ij = C_ij.view(N, M, 1)
    a_i = a_i.expand(-1, M, -1)
